...
外部补充最多提出 3 个由论文产生的问题，通常只进行两次网页搜索；第三次仅是本地流边界的硬性安全上限，不是必须完成的配额。它没有最低来源数量：一条可靠资料已经足够解决翻译歧义时立即结束，成功检索后没有可采纳来源也可正常完成。返回 403、429、robots 限制、无法连接或没有可核验正文的候选页面会直接跳过，不写入文件，也不在侧栏显示“来源受限”。只有整个 Codex 请求、响应协议、结果解析或文件写入失败时，外部阶段才显示警告。
...
- `background-sources.json` 保存检索问题、已采纳网址、来源等级，以及请求、协议、解析或写入阶段的错误；单个候选页面的访问失败不会进入该文件。
...
网页搜索不是翻译前置条件，也不绑定 Crossref 或 Semantic Scholar。插件不要求凑够固定数量的来源：已有资料足以消除翻译歧义时立即结束，零个或一个采纳来源都可以是正常结果。不可访问或无法核验的候选页面直接过滤；只有整个请求、协议、解析或写入失败才记为 `warning`。只有能与本次 Responses 流中的 URL citation 精确对应的 HTTPS 来源才会写入文件；没有可核验来源时不会保存模型记忆中的背景断言。背景内容只帮助模型消歧，不会被添加到译文里。
